    ...
    void loop()
    {
        ...
        case 4:
            ...
            if (((millis() - startPlayPrevNote) > pauseBetweenNotes))
            {
                ...
                if (thisNote >= melodyList.size()) // если сиграли последнюю ноту
                {
                    if (_cycle)
                    {
                        thisNote = 0;
                ...
                if (!pause) // находим абсолютный интервал у ноты, паузу пропускаем
                {
                    ...
                    pause = false;
                    int relIntervalC0 = int(intervalC0) - int(prevIntervalC0); // находим интервал от предидущей ноты
                    float intervalA4 = 69 - int(intervalC0);
                    int monoFreqToPlay = A4 / pow(2.0, (intervalA4 / 12)); // растчет частоты по монохроматической гамме
                    ...
                    else
                    {
                        ...
                        // далее расчет частоты новой ноты в природном интервале от предидущей
                        if (relIntervalC0 > 0)
                        {
                        ...
                        // это прозводит к рассогласованию нот и убеганию в конце произведения
                        if (freqToPlay < monoFreqToPlay) // делаем коррективовку убегания с помощью выбора малых или больших секунд
                        {
